<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Pengisian Bensin</h2>
    </x-slot>

    <div class="py-6 max-w-lg">
        <form method="POST" action="{{ route('fuel-logs.update', $fuelLog) }}" class="bg-white p-6 rounded-lg shadow-sm border border-gray-200 space-y-4">
            @csrf @method('PUT')
            <div>
                ...
                <select name="item_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    @foreach ($items as $item)
                        <option value="{{ $item->id }}" {{ old('item_id', $fuelLog->item_id) == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="fuel_date" value="{{ old('fuel_date', $fuelLog->fuel_date->format('Y-m-d')) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            ...
            <div>
                ...
                <select name="fuel_type_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    @foreach ($fuelTypes as $ft)
                        <option value="{{ $ft->id }}" {{ old('fuel_type_id', $fuelLog->fuel_type_id) == $ft->id ? 'selected' : '' }}>{{ $ft->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="amount" value="{{ old('amount', $fuelLog->amount) }}" step="0.01" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            ...
            <div>
                ...
                <input type="number" name="liter" value="{{ old('liter', $fuelLog->liter) }}" step="0.001" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                @error('liter') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Catatan</label>
                <input type="text" name="notes" value="{{ old('notes', $fuelLog->notes) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            </div>
